...
export async function createTournament<T extends Prisma.TournamentCreateArgs>(
  ...
  return prisma.tournament.create(args)
    .then(Success)
    .catch(InternalErrorFailure);
}


export async function findFirstTournament<T extends Prisma.TournamentFindFirstArgs>(
  args: Prisma.SelectSubset<T, Prisma.TournamentFindFirstArgs>
): Promise<Result<Prisma.TournamentGetPayload<T>, 'record-not-found'>> {
  return prisma.tournament.findFirst(args)
    .then((tournament) => {
      if (!tournament) {
        return Failure(
          'record-not-found',
...
export async function findManyTournament<T extends Prisma.TournamentFindManyArgs>(
  args?: Prisma.SelectSubset<T, Prisma.TournamentFindManyArgs>
): Promise<Result<Array<Prisma.TournamentGetPayload<T>>>> {
  return prisma.tournament.findMany(args)
    .then(Success)
    .catch(InternalErrorFailure);
}
...
export async function updateTournament<T extends Prisma.TournamentUpdateArgs>(
  ...
  return prisma.tournament.update(args)
    .then(Success)
