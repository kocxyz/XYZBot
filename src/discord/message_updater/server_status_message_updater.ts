...
export const ServerStatusMessageUpdater = {
  update: async (client) => {
    const channel = await client.channels
      .fetch(environment.DISCORD_SERVER_STATUS_CHANNEL_ID)
      .catch((error) => {
        logger.error(
    ...
    const message = await channel.messages
      .fetch(environment.DISCORD_SERVER_STATUS_MESSAGE_ID)
      .catch((error) => {
        logger.error(
          `Could not fetch messages from Server Status Channel: ${JSON.stringify(
            error,
          )}`,
        );
      });
    ...
    if (!message?.id) {
      await channel
        .send({
          embeds: [await createServerStatusEmbed(client.user!.avatarURL()!)],
        })
        .catch((error) => {
          logger.error(
